...
# Ensure directory to copy has trailing slash for rsync
[[ "$DIR_TO_COPY" != */ ]] && DIR_TO_COPY="$DIR_TO_COPY/"
...
# Copy directory to each host in the array using rsync
for HOST in "${HOSTS_ARRAY[@]}"; do
  echo "Copying $DIR_TO_COPY to $HOST:$DEST_PATH..."
  if rsync -avz "$DIR_TO_COPY" "$HOST:$DEST_PATH"; then
